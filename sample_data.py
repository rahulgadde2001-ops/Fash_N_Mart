...
def get_current_user(
    ...
    # locked accounts cannot use existing access tokens.
...
def require_any_role(*allowed_roles):
    def dependency(user: User = Depends(get_current_user)):
        ...
        inherited_roles = ROLE_HIERARCHY.get(
            role,
            {role},
        )

        if not any(
            allowed_role in inherited_roles
            for allowed_role in allowed_roles
        ):
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
...
def require_all_roles(*required_roles):
    def dependency(user: User = Depends(get_current_user)):
        ...
        if not all(
            required_role in inherited_roles
            for required_role in required_roles
        ):
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
...
class TokenCache:
    ...
    def invalidate_user(self, user_id: int) -> None:
        """
        Remove every cached token belonging to this user.
